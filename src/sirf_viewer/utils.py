...
def get_optimal_window_level(
    data: Any, percentile: float = 98.0
) -> Tuple[float, float]:
    """
...
def create_thumbnail(
    data: Any, size: Tuple[int, int] = (128, 128), indices: Optional[List[int]] = None
) -> np.ndarray:
    """
    ...
    # Normalize to 0-255
    slice_data_norm = (
        (slice_data - slice_data.min()) / (slice_data.max() - slice_data.min()) * 255
    ).astype(np.uint8)
...
def batch_process_files(
    file_pattern: str, output_dir: str, operation: str = "thumbnail", **kwargs
) -> List[str]:
    """
        ...
        try:
            ...
            if operation == "thumbnail":
                ...
                output_path = os.path.join(
                    output_dir, f"{os.path.basename(file_path)}_thumb.png"
                )
                ...
                output_path = os.path.join(
                    output_dir, f"{os.path.basename(file_path)}.gif"
                )
                ...
                info = get_data_info(data)
                output_path = os.path.join(
                    output_dir, f"{os.path.basename(file_path)}_info.txt"
                )
