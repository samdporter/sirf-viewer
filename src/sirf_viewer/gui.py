...
class SIRFViewerGUI(QMainWindow):
        ...
        if filename:
            try:
                ...
                QMessageBox.critical(
                    self, "Error", f"Failed to load ImageData: {str(e)}"
                )
    ...
    def load_acquisition_data(self):
        ...
        filename, _ = QFileDialog.getOpenFileName(
            ...
            "",
            "AcquisitionData files (*.hs);;All files (*)",
        )
        ...
        if filename:
            try:
                ...
                QMessageBox.critical(
                    self, "Error", f"Failed to load AcquisitionData: {str(e)}"
                )
                self.current_data = None
    ...
    def setup_dimension_controls(self):
        ...
        self.create_dimension_slider(
            scroll_dim, f"Scroll {self.viewer.dimension_names[scroll_dim]}"
        )
    ...
    def create_dimension_slider(self, dim_idx, label):
        ...
        slider.valueChanged.connect(
            lambda val, idx=dim_idx: self.update_dimension(idx, val)
        )
