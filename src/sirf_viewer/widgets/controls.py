...
class DimensionControlWidget(QWidget):
    ...
    def set_dimensions(
        self, dimensions: List[int], dimension_names: Optional[List[str]] = None
    ):
        """
        ...
        self.dimension_names = dimension_names or [
            f"Dim {i}" for i in range(len(dimensions))
        ]
        ...
        for i, (size, name) in enumerate(zip(self.dimensions, self.dimension_names)):
            ...
            slider.valueChanged.connect(
                lambda val, idx=i: self.on_dimension_changed(idx, val)
            )
